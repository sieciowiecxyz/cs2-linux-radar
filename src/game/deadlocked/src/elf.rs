...
pub fn file_offset_for_virtual_addr(bytes: &[u8], virtual_addr: u64) -> Result<Option<u64>> {
...
pub fn find_symbol_virtual_addr(bytes: &[u8], symbol_name: &str) -> Result<Option<u64>> {
    let file = object::File::parse(bytes).context("failed to parse object file")?;
